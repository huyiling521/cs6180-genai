...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(
            in_channels=self.model_embeddings.embed_size, 
            out_channels=self.model_embeddings.embed_size,
            kernel_size=2,
            padding='same'
        )
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ###             - Use the step function below to compute the the Decoder's next (cell, state) values
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t_unsq = alpha_t.unsqueeze(1)
        a_t = torch.bmm(alpha_t_unsq, enc_hiddens) 
        a_t = a_t.squeeze(1)
